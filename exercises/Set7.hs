...
add e (Set s) = if e `elem` rest then Set s else Set (first ++ (e:rest))
  where
    first = Prelude.takeWhile (< e) s
    rest = Prelude.dropWhile (< e) s
...
data State = Start | EggsAdded | SugarAdded | FlourAdded | AllIngredients | Mixed | Error | Finished
  deriving (Eq,Show)
...
average (n :| []) = n
average xs = (Prelude.sum xs) / fromIntegral (Prelude.length xs)
...
passwordAllowed pwd req =
  case req of
    MinimumLength n -> Prelude.length pwd >= n
    ContainsSome chars-> not . null $ Data.List.intersect pwd chars
    DoesNotContain chars-> null (Data.List.intersect pwd chars)
...
literal n = Literal (fromIntegral n)
...
render expr = 
  case expr of
    Literal n -> show n
    Add expr1 expr2 -> "(" ++ render expr1 ++ "+" ++ render expr2 ++ ")"
    Multiply expr1 expr2 -> "(" ++ render expr1 ++ "*" ++ render expr2 ++ ")"
