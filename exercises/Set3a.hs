...
capitalize xs = unwords $ map capitalizeFirst (words xs)
    where 
        capitalizeFirst (y:ys) = toUpper y : ys
...
-- powers k max = [k^n | n <- [0 .. max], k^n < max]
...
sumRights = sum . map func
    where
        func = either (const 0) id
