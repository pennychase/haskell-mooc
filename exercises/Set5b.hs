...
treeMax t =
  case t of
    Empty -> 0
    Node x l r -> max x (max (treeMax l) (treeMax r))
...
allValues condition tree = 
  case tree of
    Empty -> True
    Node x l r -> condition x && allValues condition l && allValues condition r
...
isOrdered t = 
  case t of
    Empty -> True 
    Node v l r -> allValues (<v) l && allValues (>v) r && isOrdered l && isOrdered r
...
search val (Node v l r)
  | val == v = Just []
  | otherwise = 
      case search val l of
        Just xs -> Just (StepL:xs)
        Nothing ->
          case search val r of
