...
count z ys = foldr (\x y -> if x == z then (1 + y) else y) 0 ys
...
inBoth xs ys = foldr (\x z -> if x `elem` ys then x:z else z) [] xs
...
instance Foldable List where
  foldr f z Empty = z
  foldr f z (LNode y rest) = f y (foldr f z rest)
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node n r l) = Node (f n) (fmap f r) (fmap f l)
