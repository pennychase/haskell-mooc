...
longest xs = minimum $ head $ groupBy (\x y -> length x == length y) (sortBy (comparing (Down . length)) xs)

-- Non-Sorting version
longest' xs  = minimum . map snd $ filter (\y -> fst y == maxLen) ys
    where
        ys = zip (map length xs) xs
        maxLen = maximum . map fst $ ys
...
incrementKey key alist = map (\(k, v) -> if k == key then (k, v + 1) else (k, v)) alist
...
transfer from to amount bank = 
    if Map.notMember to bank || Map.notMember from bank
        then bank
        else transfer' amount
    where
        transfer' amt
            | amt < 0 = bank
            | amt > fromJust (Map.lookup from bank) = bank
            | otherwise = Map.adjust (\x -> x - amt) from (Map.adjust (+ amt) to bank)
