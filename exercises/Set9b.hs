...
prettyPrint n qs = 
    foldr (\r rs -> makeRow r ++ rs) ""  [1..n]
    where
        makeRow r = foldr (\c ps -> cell (r, c) : ps) "\n" [1 .. n] 
        cell p = if p `elem` qs then 'Q' else '.'
...
danger' candidate queen = any (\f -> f candidate queen) [sameRow, sameCol, sameDiag, sameAntidiag]   
...
fixFirst _ [] = Nothing
fixFirst n s@(first:rest)
    | snd first > n = Nothing 
    | danger first rest = fixFirst n (nextCol first : rest)
    | otherwise = Just s
...
continue [] = []
continue s@(top : _) = nextRow top : s

backtrack :: Stack -> Stack
backtrack [] = []
backtrack [x] = []
backtrack (_:newTop:rest) = nextCol newTop : rest
